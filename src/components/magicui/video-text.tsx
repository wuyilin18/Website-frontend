...
export function VideoText({
  ...
  useEffect(() => {
    const updateSvgMask = () => {
      ...
      const newSvgMask = `<svg xmlns='http://www.w3.org/2000/svg' width='100%' height='100%'><text 
        x='50%' 
        y='50%' 
        ...
        filter='${textShadow !== "none" ? `url(#shadow)` : ""}'
      >${content}</text>
      ${
        textShadow !== "none"
          ? `
        <filter id="shadow" x="-20%" y="-20%" width="140%" height="140%">
          <feDropShadow dx="2" dy="4" stdDeviation="4" flood-color="black" flood-opacity="0.7"/>
          <feDropShadow dx="-1" dy="-1" stdDeviation="1" flood-color="black" flood-opacity="0.3"/>
          <feDropShadow dx="0" dy="6" stdDeviation="2" flood-color="black" flood-opacity="0.5"/>
        </filter>
      `
          : ""
      }
