...
const ClientOnlyMusicPlayer = () => {
  ...
  useEffect(() => {
    ...
    if (savedPosition) {
      try {
        ...
        console.error("Could not parse saved position");
      }
    } else {
      // 默认位置在右下角
      setPosition({
  ...
  // 检查边界
  const checkBoundary = (info: { point: { x: number; y: number } }) => {
    const newX = Math.max(
      0,
      Math.min(window.innerWidth - (miniMode ? 60 : 120), info.point.x)
    );
    const newY = Math.max(
      0,
      Math.min(window.innerHeight - (miniMode ? 60 : 120), info.point.y)
    );
  ...
  return (
    <MotionDiv
      ref={containerRef}
      className={cn(
        "fixed z-50 cursor-grab active:cursor-grabbing rounded-full shadow-lg bg-white dark:bg-gray-800 flex flex-col items-center justify-center overflow-hidden",
        miniMode ? "w-[60px] h-[60px]" : "w-[120px] h-[120px]"
      )}
      drag
      dragMomentum={false}
      onDragEnd={(_, info) => {
        const boundedPosition = checkBoundary(info);
...
export const MusicPlayer = dynamic(
  () => Promise.resolve(ClientOnlyMusicPlayer),
  {
    ssr: false,
  }
);
